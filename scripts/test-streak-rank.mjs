...
console.log("\n═══ 2. วันพัก: ไม่นับเพิ่ม แต่ต้องไม่ตัด ═══");
{
  ...
  // นับเฉพาะวันที่ฝึกจริง = 1 · วันพัก 2 วันข้ามไปเฉยๆ ไม่บวกไม่ตัด
  eq("ฝึกแล้วตามด้วยวันพัก 2 วัน = สตรีค 1 (วันพักไม่บวก)", computeStreak(d).current, 1);
}
{
  // ฝึกวันของ 3 วันก่อน · วันของเมื่อวานเป็นวันฝึกแต่ไม่ได้ฝึก -> ต้องตัด
  const d = mk(
    [
      { id: "a", name: "Barbell Squat", day: slotBack(3) },
      { id: "b", name: "Deadlift", day: slotBack(1) },
    ],
    { a: [sess(dk(3), [st(100, 5)])] },
  );
  eq("พลาดวันฝึกเมื่อวาน = สตรีคขาด", computeStreak(d).current, 0);
}
{
