...
- **โค้ดย้ายข้อมูลมี 2 รูปแบบ ต้องอ่านได้ทั้งคู่ตลอดไป** (ดู [src/lib/transferCode.ts](src/lib/transferCode.ts))
  — `RFZ1:` นำหน้า = gzip (ตั้งแต่ v3.14.0) · ไม่มีหัว = base64 ของ JSON ดิบแบบเดิม
  คนที่เก็บโค้ดเก่าไว้ในแชท/โน้ตต้องกู้ได้เสมอ ห้ามถอดตัวถอดรหัสแบบเก่าออก
- เขียน localStorage ตรงๆ ใต้แอปที่กำลังรันแล้ว **ต้องรีโหลดหน้าทันที** ไม่งั้น
  `useEffect(() => store.save(data))` ใน App.tsx จะเซฟ state เก่าในหน่วยความจำทับที่เพิ่งเขียนไป
...
| `test-transfercode` | โค้ดย้ายข้อมูลต้องสั้นพอส่งไลน์ (~10,000 ตัวอักษร) **และโค้ดเก่าต้องกู้ได้ตลอดไป** |
| `test-load` | งานจริงรายสัปดาห์ · **ปริมาณสูงแบบคงที่ต้องอ่านว่า "ปกติ" ไม่ใช่ "อันตราย"** |
| `test-skip` | ข้ามท่าเฉพาะวันนี้ — **ไม่ใช่การลบท่า** · พรุ่งนี้กลับมาเอง · ท่าที่ข้ามไม่ถูกนับในความคืบหน้า |
| `test-recgain` | คำแนะนำที่โชว์ต้อง **ทำแล้วคะแนนขยับจริง** · ตัวเลขบนการ์ดต้องตรงกับที่กดแล้วได้ |
